...
public class OpenLog {
  ...
  private boolean openLogExecute(String command, StringBuffer reply, char promptChar) {
    ...
    while (true) {
      ...
      if ((ch != (char)EscChar) && ((ch != 0) && (ch != -1)) && (count < capacity)) {
        if (fileIsOpen && ((pos == 0 && ch == '\r') || (pos == 1 && ch == '\n'))) {
          /* do not add \r\n at the beginnig when we read from file */
        } else {
          count++;
          buff.append(ch);
        }
  ...
  private boolean listDirStart() {
    ...
    if (openLogExecute("efcount", null, shellReady)) {
      ...
      if (stringSplitter.splitString(ch, '|', tempBuffer.length()) == 2) {
          try {
            // Add the characters
            opBuffer.clear();
            for (int i = stringSplitter.stringSplit[1].startIndex; i < stringSplitter.stringSplit[1].endIndex; i++) {
      ...
      if (openLogExecute(opBuffer.toString(), null, shellReady)) {
        ...
        if (stringSplitter.splitString(ch, '|', tempBuffer.length()) == 2) {
          try {
            ...
            for (int i = stringSplitter.stringSplit[0].startIndex; i < stringSplitter.stringSplit[0].endIndex; i++)
  ...
  public boolean openLogResponds() {
    boolean b = false;
    // Send message
    opBuffer.clear();
    opBuffer.append(CR);
    txUart.sendString(opBuffer.toString());
    CPU.delay(500);
    while (rxUart.byteAvailable()) {
      char ch = (char)rxUart.receiveByte();
      if (ch == shellReady) {b = true;}
    }
